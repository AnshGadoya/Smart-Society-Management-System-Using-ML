...
function StaffManagementPage() {
    ...
    useEffect(() => {
        const fetchStaff = async () => {
            try {
                ...
                console.log("data getStaff", data);
    ...
    const handleAddStaff = async (staffData) => {
        try {
            const response = await staffApi.addStaff(staffData);
            const addedStaff = response.staff || response;
            console.log("✅ Added Staff:", addedStaff);

            setStaffMembers((prev) => [...prev, addedStaff]);
            setShowForm(false);
        } catch (err) {
            console.error("Add Staff Error:", err);
            setError("Failed to add staff");
    ...
    return (
        <div className='container mt-4'>
            <PageHeader PageTitle='Staff Management'
                        buttonLabel={showForm ? "Close Form" : "Add Staff"}
                        onButtonClick={() => setShowForm(!showForm)}/>

            <button
                className="btn btn-dark d-flex justify-content-between align-content-center"
                style={{color: "gold"}}
